...
class TwoMlExample:
    ...
    def __init__(self, data, parameters):
        ...
        self.model, self.model_bis, self.sc = None, None, None
        ...
        if self.train_mode:
            ...
            self.model_bis = parameters["model_bis"]
    ...
    def train_model(self):
        ...
        self.data_train = quantile_signal(self.data_train, self.look_ahead_period, pct_split=1.00)
        ...
        split = int(len(self.data_train) * pct_split)
        ...
        X_test_sc = sc.fit_transform(X_test) # It is not an error, we use fit_transform, not transform only
        ...
        # 2nd model
        X_test["1st_model_pred"] = ml_model.predict(X_test_sc)
        X_test = pd.concat((X_test,y_test), axis=1).dropna()

        list_X_bis = self.list_X.copy()
        list_X_bis.append("1st_model_pred")
        X_train_bis = X_test[list_X_bis]
        y_train_bis = X_test[list_y]

        ml_model_bis = LinearSVC()
        ml_model_bis.fit(X_train_bis, y_train_bis)

        # Save models as attributes
        self.model = ml_model
        self.model_bis = ml_model_bis
        self.sc = sc

        self.output_dictionary["model"] = ml_model
        self.output_dictionary["model_bis"] = ml_model_bis
    ...
    def get_predictions(self):
        ...
        # 1st model
        predict_array = self.model.predict(X_sc).reshape(-1,1)

        # Features for 2nd model
        X_sc_bis = np.concatenate((X_sc, predict_array), axis=1)

        # Prediction 2nd model
        predict_array_bis = self.model_bis.predict(X_sc_bis)
        self.data["ml_signal"] = 0
        self.data["ml_signal"] = predict_array_bis
