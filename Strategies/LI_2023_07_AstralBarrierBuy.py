...
class AstralBarrierBuy:
    ...
    def get_features(self, data_sample):
        data_sample = astral(data_sample)
    ...
    def train_model(self):
        ...
        self.data_train = get_barrier(self.data_train, tp=.0035, sl=-.0020, buy=True)
        self.data_train["astral_shift_1"] = self.data_train["astral"].shift(1)
        self.data_train = self.data_train.loc[self.data_train["astral_shift_1"]==1]
    ...
    def get_entry_signal(self, time):
        ...
        if len(self.data.loc[:time]["ml_signal"]) < 6:
            return 0, self.entry_time

        # Create entry signal --> -1,0,1
        entry_signal = 0
        if (self.data.loc[:time]["astral"][-2] == 1) and (self.data.loc[:time]["ml_signal"][-2] == 1):
            entry_signal = 1
