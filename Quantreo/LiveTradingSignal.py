...
def li_2024_07_SvcMajorFx(symbol, timeframe, model_path, sc_path):
    df = get_rates(symbol=symbol, number_of_data=200, timeframe=timeframe)

    df = derivatives(df, "close")
    df = candle_information_bis(df)
    df = moving_yang_zhang_estimator(df, 100)
    df = moving_parkinson_estimator(df, 20)
    for i in [1, 2, 5]:
        data_sample = log_transform(df, "close", i)
    df = data_sample.fillna(value=0)
    ...
    list_X = ["velocity", "acceleration", "candle_way", 'ret_log_1', 'ret_log_2', 'ret_log_5',
